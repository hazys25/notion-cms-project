...
> ⚠️ **방향 전환 안내 (2026-06-16):** 이 프로젝트는 범용 스타터킷에서 **"관심주식 뉴스 모음"(Notion CMS 뉴스 블로그)** 으로 확정되었습니다. 이에 따라 스타터 잔재(**관리자 인증·Supabase·Zustand·React Hook Form**)를 제거했습니다. 따라서 아래 **Phase 2의 인증, Phase 3의 Supabase, Phase 4의 Zustand 항목은 더 이상 유효하지 않습니다.** 실제 제품 로드맵은 `docs/PRD.md` §8(구현 단계 1~5)을 기준으로 삼습니다. 아래 내용은 스타터킷 기준의 과거 기록으로 보존합니다.
